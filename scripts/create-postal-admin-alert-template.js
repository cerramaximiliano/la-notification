/**
 * Crea/actualiza el template `notification/postal-admin-alert` en Mongo
 * con el shell unificado + slots de banners. Idempotente.
 *
 * El código tiene un fallback equivalente (postalAdminAlertProcessor.buildFallbackHtml),
 * así que el email sale igual si este doc no existe.
 *
 * Uso: node scripts/create-postal-admin-alert-template.js
 */
...
<div style="display:none; max-height:0; overflow:hidden; mso-hide:all; font-size:1px; line-height:1px; color:#F4F5F7;">Alerta operativa del seguimiento postal.</div>
...
<table role="presentation" class="wrap" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px; max-width:600px;">
  <tr><td style="padding:0 0 18px 0;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
      ...
      <td align="right" style="vertical-align:middle;font-size:12px;color:#B91C1C;letter-spacing:0.06em;text-transform:uppercase;font-weight:600;">Alerta operativa</td>
    </tr></table>
  </td></tr>
...
(async () => {
  ...
  const res = await coll.updateOne(
    { category: 'notification', name: 'postal-admin-alert' },
    {
      $set: {
        subject: '{{subjectText}}',
        preheader: 'Alerta operativa del seguimiento postal.',
        description: 'Alerta operativa al admin: seguimientos postales sin actualizar (pipeline roto) y su recuperación',
        htmlContent: HTML, htmlBody: HTML,
        textContent: TEXT, textBody: TEXT,
        isActive: true,
        sendingSource: 'la-notification',
        variables: ['tituloPrincipal','ledeText','subjectText','alertKind','staleCount','staleAfterHours','statusCardHtml','trackingsHtml','trackingsText','ctaUrl','ctaLabel'],
        updatedAt: new Date()
      },
      $setOnInsert: { category: 'notification', name: 'postal-admin-alert', tags: [], createdAt: new Date() }
    },
    { upsert: true }
  );
