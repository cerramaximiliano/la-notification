...
function processMorningDigestData(summaries = {}) {
  ...
  // Secciones con el lenguaje visual del sistema: contenedor con banda azul
  // de título y superficie gris con filas, igual que las cards de los emails
  // de usuario.
  const seccionesHtml = bloques.map((b) => {
    const filas = b.filas(b.s);
    const err = b.s && (b.s.error || b.s.success === false);
    const cuerpo = err
      ? `<p style="margin:0;font-size:13px;line-height:1.55;color:#DC2626;">Error: ${b.s.error || 'el trabajo no completó'}</p>`
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#FFFFFF;border:1px solid #E6EAF2;border-radius:8px;margin-bottom:8px;">
              <tr><td style="padding:9px 12px;">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
                  ...
                  <td align="right" style="font-size:14px;font-weight:700;color:${value > 0 ? '#0F172A' : '#94A3B8'};">${value}</td>
                </tr></table>
              </td></tr>
            </table>`).join('');
    ...
    return `
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid #E6EAF2;border-radius:10px;overflow:hidden;margin-bottom:12px;">
        <tr><td style="background-color:${err ? '#FEF2F2' : '#EFF4FF'};padding:11px 18px;">
          <p style="margin:0;font-size:11px;color:${err ? '#DC2626' : '#3A7BFF'};letter-spacing:0.1em;text-transform:uppercase;font-weight:700;">${b.titulo}</p>
        </td></tr>
        <tr><td style="background-color:#F8FAFC;border-top:1px solid #E6EAF2;padding:14px 18px 6px 18px;">${cuerpo}</td></tr>
      </table>`;
  }).join('');
...
/**
 * Shell de FALLBACK del informe matinal (si el template de la base no está).
 * Mismo diseño que el template para que el correo no cambie de aspecto.
 */
function buildMorningDigestFallbackHtml(vars) {
...
function buildConfigSummarySection(config) {
  ...
  const WORKERS = [
    ...
    ['pjcatamarca-update-worker', 'PJ Catamarca — update', { firstSyncPolicy: 'silent-baseline' }],
    ['pjmendoza-update-worker', 'PJ Mendoza — update', { firstSyncPolicy: 'silent-baseline' }]
  ];
  const DELIVERY = [['pjn', 'PJN'], ['eje', 'EJE'], ['mev', 'MEV'], ['scba', 'SCBA'], ['pjsalta', 'PJ Salta'], ['pjcatamarca', 'PJ Catamarca'], ['pjmendoza', 'PJ Mendoza']];
